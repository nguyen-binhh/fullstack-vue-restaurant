<template>
  <div
    ...
    :class="filterOrders.length > 0 ? '' : 'fit-screen'"
  >
    <div v-if="filterOrders.length > 0" class="my-order-cards">
      <div
        v-for="b in filterOrders.slice().reverse()"
        class="card"
        :key="b.order_id"
      >
        <div
        ...
        >
          <div>
            <span>Order No - </span>
            <span>{{ b.order_id }}</span>
          </div>
          <button @click="sendBillId(b.order_id)">show order details</button>
        </div>
        ...
        >
          <div class="w-100 text-center py-1 px-2">
            <span>Paid:</span>{{ " " + b.order_paid }}
          </div>
          <div class="w-100 text-center py-1 px-2">
            <span>Status:</span>{{ " " + avaiableStatus[b.order_status] }}
          </div>
          <div class="w-100 text-center py-1 px-2">
            <span>When:</span> {{ b.order_when }}
          </div>
        </div>
        <div
          class="d-flex flex-wrap flex-sm-nowrap justify-content-between card-summary"
        >
          <div class="w-100 text-center py-1 px-2">
            <span>Total:</span> ${{ b.order_total }}
          </div>
          <div class="w-100 text-center py-1 px-2">
            <span>Address:</span>{{ " " + b.order_address }}
          </div>
          <div class="w-100 text-center py-1 px-2">
            <span>Phone:</span>{{ " " + b.order_phone }}
          </div>
        </div>
        ...
        <div class="card-body">
          ...
          >
            <div class="step" :class="b.order_status >= 1 ? 'completed' : ''">
            ...
            <div class="step" :class="b.order_status >= 2 ? 'completed' : ''">
              <div class="step-icon-wrap">
                <div class="step-icon">
            ...
            <div class="step" :class="b.order_status >= 3 ? 'completed' : ''">
              <div class="step-icon-wrap">
                <div class="step-icon">
            ...
            <div class="step" :class="b.order_status >= 4 ? 'completed' : ''">
              <div class="step-icon-wrap">
                <div class="step-icon"><i class="fa-solid fa-route"></i></div>
              </div>
              <h4 class="step-title">Delivering</h4>
            </div>
            <div class="step" :class="b.order_status >= 5 ? 'completed' : ''">
              <div class="step-icon-wrap">
    ...
    <div v-else class="box-content row no-food">
      ...
      <div>
        <img src="/images/no-orders.png" alt="" />
      </div>
      <router-link class="btn" to="/menu">Order now!</router-link>
    </div>

    <OrderDetails v-if="showOrderDetails" :order="sendId">
...
export default {
  ...
  setup() {
    ...
    const getAllOrders = async () => {
      if (user) {
        allOrder.value = (
          await axios.get("/orderstatus/user/" + user.user_id)
        ).data;
      }
    };
    ...
    const filterOrders = computed(() => {
      return allOrder.value.filter(
        (order) => order.order_status < 6 && order.order_status > 0
      );
    });

    const autoUpdate = onMounted(() => {
    ...
    const sendBillId = (id) => {
      sendId.value = id;
      showOrderDetails.value = !showOrderDetails;
    };
