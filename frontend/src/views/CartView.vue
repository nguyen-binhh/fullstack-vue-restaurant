                ...
                <div v-for="(f, index) in filterFoods" :key="index">
                  <div class="box-content row">
                    ...
                    <div class="desc col-sm-4">
                      <h2 class="item-name">{{ f.product_name }}</h2>
                      <div class="item-desc">
                        <b>Description</b>
                        <p>{{ f.product_desc }}</p>
                      </div>
                    ...
                    <div class="item-price col-sm-1">
                      <span class="sale-price"
                        >${{
                          ...
                          parseFloat(f.product_discount)
                        }}</span
                      >
                      <p
                        class="text-muted first-price"
                        v-if="parseFloat(f.product_discount) != 0.0"
                      >
                        ${{ parseFloat(f.product_price) }}
                      </p>
                    </div>
                    ...
                    <div class="cal-total col-sm-2">
                      <h4 class="item-total">
                        ${{
                          parseInt(
                            (f.product_price - f.product_discount) *
                              itemQuantity[index]
                          )
                        }}
                      </h4>
...
export default {
  setup() {
    ...
    const onQtyChange = async (e, i) => {
      if (e.target.value < 1) {
        e.target.value = 1;
        itemQuantity.value[i] = 1;
      } else {
        itemQuantity.value[i] = e.target.value;
      }
      let data = {
        user_id: parseInt(user.user_id),
        product_id: parseInt(cartItem.value[i]),
        item_qty: itemQuantity.value[i],
      };
    ...
    const removeBtn = async (index) => {
      await axios.delete(
        `/cartItem/` + user.user_id + "/" + cartItem.value[index]
      );
