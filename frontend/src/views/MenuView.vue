        ...
        <div class="row box-container">
          <div v-for="(f, index) in currentPageItems" :key="index">
            <div class="box">
              ...
              <div class="image">
                <img :src="`/images/${f.product_src}`" alt="" />
              </div>
              <div class="content">
                <h3>{{ f.product_name }}</h3>
                <div class="stars">
                  <div
                    v-for="s in Math.floor(parseFloat(f.product_star))"
                    :key="s"
                    class="d-inline"
                  >
                  ...
                  <div
                    v-if="
                      parseFloat(f.product_star) -
                        Math.floor(parseFloat(f.product_star)) ==
                      0.5
                    "
                    class="d-inline"
                  >
                ...
                <div class="price">
                  ${{
                    parseFloat(f.product_price) - parseFloat(f.product_discount)
                  }}
                  <span v-if="parseFloat(f.product_discount) != 0.0"
                    >${{ parseFloat(f.product_price) }}</span
                  >
                </div>
...
export default {
  ...
  setup() {
    ...
    const { foods, getAllFoods } = useData();
    getAllFoods();
    ...
    const filterFoods = computed(() => {
      return foods.value.filter(
        (item) =>
          item.product_name.toLowerCase().match(foodObj.name.toLowerCase()) &&
          (item.product_category.match(foodObj.category) ||
            foodObj.category == "all" ||
            foodObj.category == "") &&
          evaluatePrice(item, foodObj.price) &&
          item.product_type.toLowerCase().match(foodObj.type.toLowerCase()) &&
          evaluateStatus(item, foodObj.status)
      );
    });
    ...
    const calculatePages = computed(() => {
      if (filterFoods.value.length % perPage.value !== 0) {
        return Math.floor(filterFoods.value.length / perPage.value);
      }
    });
    ...
    function evaluatePrice(food, priceRange) {
      pageNum.value = 0;
      let cal =
        parseFloat(food.product_price) - parseFloat(food.product_discount);
      if (priceRange == "2,5") {
        if (2 <= cal && cal <= 5) {
          return food;
        }
    ...
    function checkSale(food, statusArray) {
      if (statusArray.includes("Sale Off")) {
        if (parseFloat(food.product_discount) > 0) {
          return true;
        } else {
          return false;
        }
    ...
    function checkBest(food, statusArray) {
      if (statusArray.includes("Best Seller")) {
        if (food.product_status.includes("best seller")) {
          return true;
        } else {
          return false;
        }
    ...
    function checkOnl(food, statusArray) {
      if (statusArray.includes("Online Only")) {
        if (food.product_status.includes("online only")) {
          return true;
        } else {
          return false;
        }
    ...
    function checkSeason(food, statusArray) {
      if (statusArray.includes("Seasonal Dishes")) {
        if (food.product_status.includes("seasonal dishes")) {
          return true;
        } else {
          return false;
        }
    ...
    function checkNew(food, statusArray) {
      if (statusArray.includes("New Dishes")) {
        if (food.product_status.includes("new dishes")) {
          return true;
        } else {
          return false;
        }
    ...
    function filterStatusBtn(e) {
      pageNum.value = 0;
      if (foodObj.status.includes(e.target.value) === false) {
        foodObj.status.push(e.target.value);
    ...
    function unselectStatusBtn(e) {
      pageNum.value = 0;
      foodObj.status = foodObj.status.filter((a) => a !== e.target.value);
    ...
    function addItem(index) {
      sendId.value = parseInt(currentPageItems.value[index].product_id);
