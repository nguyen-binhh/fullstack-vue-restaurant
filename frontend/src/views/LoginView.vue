...
export default {
  setup() {
    ...
    const { getMatchUser, matchUser } = useData();
    ...
    const handleSubmit = async (e) => {
      errors.value = [];
      if (!loginObj.email) {
        errors.value.push("Entering a email is required !! Please try again");
      } else {
        if (!/[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,3}$/.test(loginObj.email)) {
          errors.value.push("Email must be valid !!  Please try again");
        }
      }
      if (!loginObj.pass) {
        errors.value.push("Password is required !!  Please try again");
      }
      if (!errors.value.length === 0) {
        ...
        if (!matchUser.value) {
          errors.value.push("Incorrect email or password!!  Please try again");
        } else {
          if (matchUser.value.user_password === loginObj.pass) {
            matchUser.value.user_password = "";
            users.setUser(matchUser.value);
            Swal.fire({
              position: "top-end",
              icon: "success",
